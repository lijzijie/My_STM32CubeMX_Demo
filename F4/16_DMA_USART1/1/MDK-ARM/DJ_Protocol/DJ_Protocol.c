...
//extern u8 rx_buffer[USART2_REC_LEN];     //接收缓冲,最大USART_REC_LEN个字节.
...
/*
** Descriptions: send the data with usart
** Input: cmdid ,pchMessage , dwLength :  Message's length
** Output: NULL
*/
...
{	
	uint8_t i;
	uint8_t *addr;
	static tFrame Frame = {.FrameHeader.SOF = 0xA5};// 帧
	
	if ((pchMessage == 0) || (dwLength != sizeof(GameInfo)) || (dwLength != sizeof(RealBloodChangedData))
		  || (dwLength != sizeof(RealShootData)) || (dwLength != sizeof(tSelfDefineInfo)))
		return ;
	
	addr = (uint8_t *)&Frame.Data.GameInfo;
	
	Frame.CmdID = cmdid; //命令
	Frame.FrameHeader.DataLength = dwLength;//数据长度
	...
	USART_SendBuff(&huart1,(uint8_t *)&Frame,i);               //帧发送
}
//数据验证， 可以不调用
void DataVerify()
{
	...
	if(recv_end_flag&0x8000)//接收完成
	{		
		frame = (tFrame *)rx_buffer;//帧数据缓存地址
